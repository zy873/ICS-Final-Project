...
#==============================================================================
				...
				message_cont = msg['message']
				message_content = __flip__(message_cont)
				msgtime = text_proc(message_content, from_name)
				self.indices[from_name].add_msg_and_index(msgtime)
				# index each message
				for g in the_guys:
					to_sock = self.logged_name2sock[g]
					mysend(to_sock, json.dumps({"action":"exchange","from": '[' + from_name + ']', 'message':message_content}))
...
#==============================================================================
				...
				print(self.sonnet)
				poem = self.sonnet.get_sect(poem_num)
				print('here:\n', poem)
...
#==============================================================================
				...
				for i in self.indices.keys():
					if self.indices[i].search(msg['target']) != '':
						search_rslt += self.indices[i].search(msg['target'])
...
def __flip__(message):
	msg_list = message.split(' ')
	if '__flip__' == msg_list[0]:
		after = msg_list[1:]
		after.reverse()
		msg = [msg_list[0]] + after
		result = ' '.join(msg)
		return result
	else:
		return message
