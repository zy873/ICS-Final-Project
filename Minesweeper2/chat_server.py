...
class Server:
	def __init__(self):
		...
		self.top_grp = []
		self.top_time = []
		self.leaderboard = {} #key: time period , value: grp members' names as a list
...
#==============================================================================
#                quit the Minesweeper game pool \\\\\\\\ last change
#==============================================================================						
...
#				self.minesweeper_group.disconnect(from_name)
				...
				# if end game due to sb wins, return the end msg
				try:
					mysend(to_sock, json.dumps({"server_msg": msg['reason'],'starter': self.starter,'board':self.board, 'end_msg':self.end_msg,'leaderboard':msg['leaderboard']}))
					...
					if from_name == self.starter:
						if (pos[0] in self.short.letters) and pos[1:] <='9' and pos[1:] >= '1' and (pos not in self.short.change_frame):
							...
							if self.short.win:
								...
								period = format(self.end_time - self.start_time, '.3f')
							...
							mysend(from_sock, json.dumps({'server_msg':'successfully moved', 'starter': self.starter,'board':self.board, 'end_msg':self.end_msg,'leaderboard':leader_msg}))
							mysend(to_sock, json.dumps({'server_msg':'successfully moved','starter': self.starter,'board':self.board, 'end_msg':self.end_msg,'leaderboard':leader_msg}))	
						else:
